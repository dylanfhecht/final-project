  ...
  for(unsigned int i=0; i < _TOTAL_IMAGES; i++){
    voxelgrid.push_back((source_path + files[i]).c_str());
      /* fill to size of vertices */{
        std::size_t vertices_size = voxelgrid[i].vertices.size();
        std::size_t colors_size = voxelgrid[i].colors.size();
        std::size_t normals_size = voxelgrid[i].normals.size();
        if (colors_size < vertices_size) {
          voxelgrid[i].colors.resize(vertices_size);
          for (std::size_t j = colors_size; j < vertices_size; ++j) {
            voxelgrid[i].colors[j] = vec3(0.f,0.f,0.f);
          }
        }
        if (normals_size < vertices_size) {
          voxelgrid[i].normals.resize(vertices_size);
          for (std::size_t j = normals_size; j < vertices_size; ++j) {
            voxelgrid[i].normals[j] = vec3(1.f,1.f,1.f);
          }
        }
      }
